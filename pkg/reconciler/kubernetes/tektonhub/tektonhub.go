...
func (r *Reconciler) FinalizeKind(ctx context.Context, original *v1alpha1.TektonHub) pkgreconciler.Event {
	...
	ths, err := r.operatorClientSet.OperatorV1alpha1().TektonHubs().List(ctx, metav1.ListOptions{})
	if err != nil {
		return fmt.Errorf("failed to list all TektonHubs: %w", err)
	}

	for _, th := range ths.Items {
		if th.GetDeletionTimestamp().IsZero() {
...
func (r *Reconciler) validateOrCreateDBSecrets(ctx context.Context, th *v1alpha1.TektonHub) error {
	...
	dbSecret, err := r.getSecretForHub(ctx, th.Spec.Db.DbSecretName, namespace, dbKeys)
	if err != nil {
...
func (r *Reconciler) validateApiDependencies(ctx context.Context, th *v1alpha1.TektonHub) error {
	...
	_, err := r.getSecretForHub(ctx, th.Spec.Api.ApiSecretName, namespace, apiSecretKeys)
	if err != nil {
		if apierrors.IsNotFound(err) {
	...
	_, err = r.getConfigMapForHub(ctx, apiConfigMapName, namespace, apiConfigMapKeys)
	if err != nil {
		if apierrors.IsNotFound(err) {
...
func (r *Reconciler) validateUiConfigMap(ctx context.Context, th *v1alpha1.TektonHub) error {
	...
	namespace = os.Getenv("DEFAULT_TARGET_NAMESPACE")
	_, err := r.getConfigMapForHub(ctx, uiConfigMapName, namespace, uiConfigMapKeys)
	if err != nil {
		if apierrors.IsNotFound(err) {
...
func createUiConfigMap(name, namespace string, th *v1alpha1.TektonHub) *corev1.ConfigMap {
	return &corev1.ConfigMap{
		...
		Data: map[string]string{
			"API_URL":       "https://" + th.Status.ApiRouteUrl,
			"AUTH_BASE_URL": "https://auth.hub.tekton.dev",
			"API_VERSION":   "v1",
